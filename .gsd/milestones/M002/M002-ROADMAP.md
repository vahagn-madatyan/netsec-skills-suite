...
- [ ] **S03: Security Operations Skills** `risk:medium` `depends:[S01]`
  > After this: `bash scripts/validate.sh` passes 22 skills, SIEM skill uses `[Splunk]`/`[ELK]`/`[QRadar]` labeled query patterns, incident response skill focuses on network forensics evidence (not general IR), README catalog updated with 3 new rows
